...
char g_fileList[RLOG_MAX_FILES][MAX_FILENAME_LENGTH];
...
void rlPrintConfiguration(void)
{
	...
	fprintf(stderr, "Log level[%d]:\t\t[%s]\n",g_logLevel-1, g_logStr[g_logLevel-1]);
...
void readCircularBuffers()
{
   ...
   for(i=0; i < g_nThreadsRegistered; i++) 
   {
      ...
      if( pThrData->logReadPos < writerPos  )
      {
         ...
         if( g_nCliSocket &&  send(g_nCliSocket, pThrData->logBuff+pThrData->logReadPos, dataLen, 0 ) == -1 ) 
         {
      ...
      else if ( pThrData->logReadPos > writerPos ) 
      {
         ...
         /* If post processor connected send logs */
         if( g_nCliSocket &&  send(g_nCliSocket, pThrData->logBuff, writerPos, 0 ) == -1 ) 
         {
            closeConnection(g_nCliSocket);
            g_nCliSocket = 0;
         }
...
void rlCatchSegViolation(int sig)
{
	...
	char **strings; 
   char buf[RLOG_MAX_STACK_DEPTH*128] = {0};
      ...
      U32 arg3, U32 arg4, ...)
{
   ...
   arg.logData.len  = sizeof(U8) + sizeof(U32) + (sizeof(U32)*arg.logData.numOfArgs);
