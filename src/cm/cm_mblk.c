...
#else
PUBLIC S16 cmAllocEvntNew (evntSize,maxBlkSize,sMem,ptr)
Size            evntSize;   /* Size of the Event structure */
...
#ifdef SS_LIGHT_MEM_LEAK_STS
  if (SGetSBufNew(sMem->region, sMem->pool, 
               (Data **)&allocPtr, 
               (Size)(maxBlkSize + sizeof(CmMemList)),lineNo,funcName) != ROK)
#else
#ifdef SS_LOCKLESS_MEMORY
