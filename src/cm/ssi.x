...
extern int brcm_mem_mng_flush(void *addr, int len);
#define SHM_FREE_FLAG         0x01
#define SHM_DYNAMIC_MEM_FLAG  0x02
typedef struct shmInstruHdr
{
	  U32 trSignature;
	  U32 memFlags;
	  U32 timeStamp;
...
#define ALLOC_FUNC_COPY(x)\
{\
	  U8 *tptr = NULL;\
	  tptr = (U8 *)x;\
	  tptr = tptr - sizeof(ShmInstruHdr);\
	  ShmInstruHdr *shmInstruHdr_ap = (ShmInstruHdr *)tptr;\
	  strncpy((char *)shmInstruHdr_ap->preAllocFunc,(char *) shmInstruHdr_ap->allocFunc, 14);\
	  strncpy((char *)shmInstruHdr_ap->allocFunc,(const char *) fnName, 14);\
	  shmInstruHdr_ap->preAllocLineNum = shmInstruHdr_ap->allocLineNum;\
	  shmInstruHdr_ap->allocLineNum = line;\
	  brcm_mem_mng_flush(shmInstruHdr_ap, sizeof(ShmInstruHdr));\
}
#define FREE_FUNC_COPY(x)\
{\
	  U8 *tptr = NULL;\
	  tptr = (U8 *)x;\
	  tptr = tptr - sizeof(ShmInstruHdr);\
	  ShmInstruHdr *shmInstruHdr_fp = (ShmInstruHdr *)tptr;\
	  strncpy((char *)shmInstruHdr_fp->preFreeFunc, (char *)shmInstruHdr_fp->freeFunc, 14);\
	  strncpy((char *)shmInstruHdr_fp->freeFunc,(const char *) fnName, 14);\
	  shmInstruHdr_fp->preFreeLineNum = shmInstruHdr_fp->freeLineNum;\
	  shmInstruHdr_fp->freeLineNum = line;\
	  brcm_mem_mng_flush(shmInstruHdr_fp, sizeof(ShmInstruHdr));\
}
#endif
...
typedef S16 (*SsAlloc) ARGS((Void *, Size *, U32 , Data **, U32 ,const U8 *));
typedef S16 (*SsFree) ARGS((Void *, Data *, Size, U32,const U8 *));
typedef S16 (*SsCtl) ARGS((Void *, Event, SMemCtl *));
#else
#ifdef SSI_DEBUG_LEVEL1
...
EXTERN S16 SValidatePayloadPtr ARGS((void *pldPtr));
#ifdef QCOM_TOBE_REMOVED 
EXTERN S16 SValidateDfltRegnPtr ARGS((void *dfltRegnPtr));
#endif
EXTERN S32 SGetPayloadFlatBuf ARGS((Size size,Data **dataPtr, U32 offset));
EXTERN S32 SFreePayloadFlatBuf ARGS((Data *dataPtr));
EXTERN S16 SGetMsgUsingPtr ARGS((Buffer **mBuf,Data *dataPtr,Size  dataLen));
EXTERN S16 SGetPayloadFlatBufBaseLimit ARGS((Data *payloadDataPtr,Data *base,Data *limit));
EXTERN  Void SRouteLog ARGS((void *logbuf));
EXTERN U32 SGetUlMemMsg(Buffer **mBuf,MsgLen len);
...
#ifdef BC_CPUL_SHM_INST
EXTERN  S16 SFlushQueueNew ARGS((Queue *q,U32 line,const U8 *fnName));
#else
...
#elif defined(BC_CPUL_SHM_INST)
EXTERN  INLINE S16 SGetDBufNew ARGS((Region region,Pool pool,Buffer * *bufPtr,U32 line,const U8 *fnName));
EXTERN  INLINE S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf, U32 line, const U8 *fnName));
#else
#if defined(SS_MEM_LEAK_DETEC_LEVEL1) || defined(SS_DEBG_MEM_CORRUP_LEVEL1) || defined(SS_LIGHT_MEM_LEAK_STS)
EXTERN  S16 SGetDBufNew ARGS((Region region,Pool pool,Buffer * *bufPtr,U32 line,U8 *fnName));
EXTERN  S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf, U32 line, U8 *fnName));
#else
...
EXTERN  S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf, U32 line, U8 *fnName));
#elif defined(BC_CPUL_SHM_INST)
EXTERN  S16 SGetDBufNew ARGS((Region region,Pool pool,Buffer * *bufPtr,U32 line,const U8 *fnName));
EXTERN  S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf, U32 line,const  U8 *fnName));
#else
EXTERN  S16 SGetDBuf ARGS((Region region,Pool pool,Buffer * *bufPtr));
EXTERN  S16 SPutDBuf ARGS((Region region,Pool pool,Buffer *buf));
#endif
...
#elif defined(BC_CPUL_SHM_INST)
EXTERN  S16 SGetMsgNew ARGS((Region region, Pool pool, Buffer * *mBufPtr, U32 line,const U8 *fnName));
EXTERN  S16 SGetSBufNew ARGS((Region region,Pool pool, Data * *bufPtr, Size size, U32 line, const U8 *fnName));
EXTERN  S16 SPutMsgNew ARGS((Buffer *mBuf, U32 line,const  U8 *fnName));
EXTERN  S16 SPutSBufNew ARGS((Region region, Pool pool, Data *buf, Size size, U32 line,const U8 *fnName));
...
#ifdef BC_CPUL_SHM_INST
EXTERN  S16 SInitMsgNew ARGS((Buffer *mBuf, U32 line,const  U8 *fnName));
EXTERN  S16 SAddPreMsgNew ARGS((Data data,Buffer *mBuf,U32 line, const U8 *fnName));
EXTERN  S16 SAddPstMsgNew ARGS((Data data,Buffer *mBuf, U32 line,const  U8 *fnName));
/* ssi_x_001.main_70 - Added prototype for SAddPreMsgMultInOrder */
EXTERN  S16 SAddPreMsgMultInOrderNew ARGS((Data *src,MsgLen cnt,Buffer *mBuf,U32 line,const  U8 *fnName));
EXTERN  S16 SRemPreMsgNew ARGS((Data *dataPtr,Buffer *mBuf, U32 line,const  U8 *fnName));
#else
...
#elif defined(BC_CPUL_SHM_INST)
EXTERN  S16 SAddPreMsgMultNew ARGS((Data *src,MsgLen cnt,Buffer *mBuf,U32 line,const  U8 *fnName));
EXTERN  S16 SAddPstMsgMultNew ARGS((Data *src,MsgLen cnt,Buffer *mBuf, U32 line,const  U8 *fnName));
#else
...
#ifdef BC_CPUL_SHM_INST
EXTERN  S16 SRemPreMsgMultNew ARGS((Data *dst,MsgLen cnt,Buffer *mBuf, U32 line,const  U8 *fnName));
EXTERN  S16 SRemPstMsgMultNew ARGS((Data *dst,MsgLen cnt,Buffer *mBuf, U32 line,const  U8 *fnName));
EXTERN  S16 SRepMsgNew ARGS((Data data,Buffer *mBuf,MsgLen idx, U32 line,const  U8 *fnName));
#else
...
#ifdef BC_CPUL_SHM_INST
EXTERN  S16 SCompressMsgNew ARGS((Buffer *mBuf,  U32 line,const  U8 *fnName));
#else
...
#else
#ifdef BC_CPUL_SHM_INST
EXTERN  S16 SCpyMsgMsgNew ARGS((Buffer *mBuf, Region region, Pool pool, Buffer **dstBuf, U32 line,const  U8 *fnName));
EXTERN  S16 SAddMsgRefNew ARGS((Buffer *mBuf, Region region, Pool pool, Buffer **dstBuf, U32 line,const  U8 *fnName));
#else
...
#ifdef BC_CPUL_SHM_INST
EXTERN S16 SCpyPartMsgNew ARGS((Buffer *srcBuf, MsgLen idx, MsgLen cnt, Buffer *dstBuf, U32 line,const  U8 *fnName));
#else
...
#ifdef BC_CPUL_SHM_INST
EXTERN S16 SMovPartMsgNew ARGS((Buffer *srcBuf, MsgLen idx, Buffer *dstBuf, U32 line,const  U8 *fnName));
#else
...
EXTERN S16 SPrintProfileLog ARGS((U32 taskId));
#ifdef MSPD_MLOG_NEW
...
#ifdef SS_DEBG_MEM_CORRUP_LEVEL1
#ifdef SS_DEBG_MEM_CORRUP_PAYLOAD
EXTERN U16  cmIsFromPoolNGetSize(Data   *ptr,U16 len);
#endif /*SS_DEBG_MEM_CORRUP_PAYLOAD*/
#endif /*SS_DEBG_MEM_CORRUP_LEVEL1 */
