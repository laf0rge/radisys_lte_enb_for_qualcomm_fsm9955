...
#endif
{
   ...
   switch(selector) {
      case FALSE:
         CMCHKPK(cmPkTfuDlCqiPusch, &param->puschCqi, mBuf);
         break;
      case TRUE:
...
#endif
{
   ...
   switch(selector) {
      case TRUE:
         ...
         break;
      case FALSE:
         CMCHKUNPK(cmUnpkTfuDlCqiPusch, &param->puschCqi, mBuf);
         break;
      default :
         RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   for (i=TFU_MAX_TB-1; i >= 0; i--) {
     for(j=3; j>=0; j--) {
      CMCHKPK(SPkU8, param->hqFbk[i][j], mBuf);
     }
   }
   ...
   for (i=TFU_MAX_TB-1; i >= 0; i--) {
     for(j=3; j>=0; j--) {
        CMCHKPK(SPkU32, param->hqFbk[i][j], mBuf);
     }
   }
...
#ifndef TFU_UPGRADE
   for (i=TFU_MAX_TB-1; i >= 0; i--) {
      CMCHKPK(SPkU8, param->isAck[i], mBuf);
   }
#endif
#ifdef TFU_UPGRADE
  for (i=TFU_MAX_TB-1; i >= 0; i--) {
...
/* ACC-TDD ccpu00130520 */
...
#ifndef TFU_UPGRADE
   for (i=0; i<TFU_MAX_TB; i++) {
      CMCHKUNPK(SUnpkU8, &param->isAck[i], mBuf);
   }
   ...
   for (i=0; i<TFU_MAX_TB; i++) {
      for(j=0; j<4; j++) {
         CMCHKUNPK(SUnpkU32, (U32 *)&param->hqFbk[i][j], mBuf);
      }
   }
    ...
    for (i=0; i<TFU_MAX_TB; i++) {
      for (j=0; j < 4; j++) {
         CMCHKUNPK(SUnpkU8, &param->hqFbk[i][j], mBuf);
      }
   }
#endif  /* TFU_UPGRADE */
...
#endif
{
   ...
   CMCHKUNPK(SUnpkU8, &param->ri, mBuf);
   RETVALUE(ROK);
}
...
#endif
{
   ...
   for (i=TFU_MAX_TDD_HQ_RES-1; i >= 0; i--) {
...
#endif
{
   ...
   for (i=0; i<TFU_MAX_TDD_HQ_RES; i++) {
...
#endif
{
   ...
   CMCHKPK(SPkU8, param->hqSz, mBuf);
   CMCHKPK(SPkU16, param->pucchIdx, mBuf);
   RETVALUE(ROK);
}
...
#endif
{
   ...
   CMCHKUNPK(SUnpkU16, &param->pucchIdx, mBuf);
   CMCHKUNPK(SUnpkU8, &param->hqSz, mBuf);
   RETVALUE(ROK);
}
...
#endif
{
   ...
   CMCHKPK(cmPkTknU8, &param->riSz, mBuf);
   CMCHKPK(SPkU8, param->cqiPmiSzRn1, mBuf);
   CMCHKPK(SPkU8, param->cqiPmiSzR1, mBuf);
   RETVALUE(ROK);
}
...
#endif
{
   ...
   CMCHKUNPK(SUnpkU8, &param->cqiPmiSzR1, mBuf);
   CMCHKUNPK(SUnpkU8, &param->cqiPmiSzRn1, mBuf);
   CMCHKUNPK(cmUnpkTknU8, &param->riSz, mBuf);
