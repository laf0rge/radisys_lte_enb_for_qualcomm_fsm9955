...
PRIVATE S16 buildRecvBufNew ARGS((CmInetMemInfo *info, MsgLen len, 
                               CmInetIovec rxArr[], Buffer *dBuf[], U16 maxSize,
                               struct msghdr *msg, Bool isStrmMsg, U32 line, const U8 *fnName));
PRIVATE S16 buildRecvMsgNew ARGS((CmInetMemInfo *info, CmInetIovec rxArr[], 
                               S16 numBduf, MsgLen msgLen, Buffer *dBufs[], 
                               Buffer **mPtr, U32 line,const U8 *fnName));
#else
/* Added another function parameter */
