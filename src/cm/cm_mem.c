...
PRIVATE S16 cmAllocNew ARGS((Void *regionCb, Size *size, U32 flags, Data **ptr, U32 line, const U8 *fnName));
PRIVATE S16 cmFreeNew  ARGS((Void *regionCb, Data *ptr, Size size, U32 line, const U8 *fnName));
#else
PRIVATE S16 cmAlloc ARGS((Void *regionCb, Size *size, U32 flags, Data **ptr));
PRIVATE S16 cmFree  ARGS((Void *regionCb, Data *ptr, Size size));
#endif /* SSI_DEBUG_LEVEL1 */
...
/* Initialize the memory manager function handlers */
...
#endif
  {
     ...
     /* Initialize the memory manager function handlers */
     regCb->regInfo.alloc = cmAllocNew;
     regCb->regInfo.free  = cmFreeNew;
#else
     /* Initialize the memory manager function handlers */
...
/*cm_mem_c_001.main_23 Fix for specifier mismatch warnings in 64BIT compilation*/          
   ...
   {
      if ( (*size <= regCb->bktMaxBlkSize) && (regCb->region ==1))
      {
         U32 availMem;
         QU_CRITLOG(QU_LOG_MESSAGE,"Memorry exhausted region1 size req = %ld",*size);
         QU_CRITLOG(QU_LOG_MESSAGE,"regCb->region = %ld",regCb->region);
         QU_CRITLOG(QU_LOG_MESSAGE,"regCb->bktMaxBlkSize = %ld",regCb->bktMaxBlkSize);
         QU_CRITLOG(QU_LOG_MESSAGE,"regCb->numBkts = %ld",regCb->numBkts);
