...
**********************************************************/
#ifdef ANSI
PUBLIC S16 cmPkRgrCellPfs
(
RgrCellPfs *param,
Buffer *mBuf
)
#else
PUBLIC S16 cmPkRgrCellPfs(param, mBuf)
RgrCellPfs *param;
Buffer *mBuf;
#endif
{
...
**********************************************************/
#ifdef ANSI
PUBLIC S16 cmUnpkRgrCellPfs
(
RgrCellPfs *param,
Buffer *mBuf
)
#else
PUBLIC S16 cmUnpkRgrCellPfs(param, mBuf)
RgrCellPfs *param;
Buffer *mBuf;
#endif
{
   S32 idx;
   TRC3(cmUnpkRgrCellPfs)
...
#endif
{
   ...
   CMCHKUNPK(SUnpkU32, (U32 *)&enumParam, mBuf);
   param->status = enumParam;
   RETVALUE(ROK);
...
#ifdef TFU_UPGRADE   
   CMCHKUNPK(SUnpkU32, (U32*)&pLow, mBuf);
   param->pwrThreshold.pLow = pLow;
   CMCHKUNPK(SUnpkU32, (U32*)&pLow, mBuf);
   param->pwrThreshold.pHigh = pLow;
#endif   
   CMCHKUNPK(SUnpkU32, (U32*)&param->status, mBuf);
...
/* rgr_c_001.main_3: Added TTI indication from MAC to RGR user */
   ...
   CMCHKPK(SPkU8, param->dlfsSchdType, mBuf);
      switch(param->ulSchdType) {
         case RGR_SCH_TYPE_PFS:
            CMCHKPK(cmPkRgrCellPfs, &param->ulSchInfo.cellUlPfs, mBuf);
            break;
         default :
            break;
      }
   CMCHKPK(SPkU8, param->ulSchdType, mBuf);
      switch(param->dlSchdType) {
         case RGR_SCH_TYPE_PFS:
            CMCHKPK(cmPkRgrCellPfs, &param->dlSchInfo.cellDlPfs, mBuf);
            break;
         default :
            break;
      }
...
#endif
{
   ...
   CMCHKUNPK(SUnpkS8, &param->pMax, mBuf);
      switch(param->dlSchdType) {
         case RGR_SCH_TYPE_PFS:
            CMCHKUNPK(cmUnpkRgrCellPfs, &param->dlSchInfo.cellDlPfs, mBuf);
            break;
         default :
            break;
      }
   CMCHKUNPK(SUnpkU8, &param->ulSchdType, mBuf);
      switch(param->ulSchdType) {
         case RGR_SCH_TYPE_PFS:
            CMCHKUNPK(cmUnpkRgrCellPfs, &param->ulSchInfo.cellUlPfs, mBuf);
            break;
         default :
            break;
      }
